...
codegenはディレクティブを考慮した型生成をしない。例えば`@auth(policy: ADMIN)`のようなディレクティブをフィールドへ付与しても、リゾルバーのコンテキスト型はAdminContextとはならない。この例の場合、SDLを優先するならリゾルバーでの型アサーションか設定ファイルでの型指定、リゾルバーを優先するならSDLへの説明の複製が必要になる。

## ディレクティブについて

あまり詳しくないが、実装が複雑で認知コストが高くテストも大変という認識。できれば避けたい。
