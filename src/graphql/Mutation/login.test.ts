...
import type { NewRefreshToken } from "../../infra/datasources/_shared/types.ts";
...
import type { MutationLoginArgs } from "../_schema.ts";
...
describe("logic", () => {
  ...
  it("retains latest 5 refresh tokens", async () => {
    ...
    expect(after.map((a) => a.createdAt).sort()[0]).toEqual(
      new Date(`2026-01-01T00:00:00.00${1}Z`),
    );
  });
});
