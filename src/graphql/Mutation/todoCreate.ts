...
const parseTitle = (args: MutationTodoCreateArgs) => {
  const result = Todo.Title.parse(args.title);

  return Array.isArray(result)
    ? result.map((e) => new ParseErr("title", e)) //
    : result;
};

const parseDescription = (args: MutationTodoCreateArgs) => {
  const result = Todo.Description.parse(args.description);

  return Array.isArray(result)
    ? result.map((e) => new ParseErr("description", e)) //
    : result;
};
...
const logic = async (
  ...
  const result = await ctx.repos.todo.add(todo);
  switch (result) {
    case "Ok":
      break;
    case "Failed":
...
if (import.meta.vitest) {
  ...
  const valid = {
    ...
    } as Exclude<ReturnType<typeof parseArgs>, Error[]>,
