...
import { resolver } from "./status.ts";
...
describe("authorization", () => {
  ...
  it("rejects when user is not owner", async () => {
    ...
    await expect(status(ctx, parent)).rejects.toSatisfy(
      (e) =>
...
describe("logic", () => {
  it("returns parent status", async () => {
    const ctx = context.alice();
    const parent: ResolversParentTypes["Todo"] = dto.todos.alice1;

    const result = await status(ctx, parent);
