...
import { resolver } from "./description.ts";
...
describe("authorization", () => {
  ...
  it("rejects when user is not owner", async () => {
    ...
    await expect(description(ctx, parent)).rejects.toSatisfy(
      (e) =>
...
describe("logic", () => {
  it("returns parent description", async () => {
    const ctx = context.alice();
    const parent: ResolversParentTypes["Todo"] = dto.todos.alice1;

    const result = await description(ctx, parent);
