...
import { resolver } from "./updatedAt.ts";
...
describe("authorization", () => {
  ...
  it("rejects when user is not owner", async () => {
    ...
    await expect(updatedAt(ctx, parent)).rejects.toSatisfy(
      (e) =>
...
describe("logic", () => {
  it("returns parent updatedAt", async () => {
    const ctx = context.alice();
    const parent: ResolversParentTypes["Todo"] = dto.todos.alice1;

    const result = await updatedAt(ctx, parent);
    expect(result?.getTime()).toEqual(Date.parse(graph.todos.alice1.updatedAt));
  });
});
