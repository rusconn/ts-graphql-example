...
describe("authorization", () => {
  ...
  it("rejects when user is not owner", async () => {
    ...
    await expect(name(ctx, parent)).rejects.toSatisfy(
      (e) =>
...
describe("logic", () => {
  it("returns parent name", async () => {
    const ctx = context.alice();
    const parent: ResolversParentTypes["User"] = dto.users.alice;

    const result = await name(ctx, parent);
