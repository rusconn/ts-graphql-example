...
const batchGet = (db: Kysely<DB>) => async (keys: readonly Key[]) => {
  const blockerIds = keys.map((key) => key.blockerId);
    ...
    .innerJoinLateral(
      (eb) =>
        eb
          ...
          .innerJoin("Block as b", "u.id", "b.blockeeId")
          .whereRef("User.id", "=", "b.blockerId")
          .$if(cursor != null, (qb) =>
  ...
  return sortGroup(blockerIds, formattedBlockings, (blocking) => blocking.cursor.blockerId);
};
