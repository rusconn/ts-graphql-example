...
import type { TodoUpdatedAtQuery, TodoUpdatedAtQueryVariables } from "../_schema.ts";

const executeQuery = executeSingleResultOperation<TodoUpdatedAtQuery, TodoUpdatedAtQueryVariables>(
  /* GraphQL */ `
...
test("owned", async () => {
  ...
  expect(data.node.updatedAt).toBe(graph.todos.admin1.updatedAt?.toISOString());
});
...
test("not owned, but admin", async () => {
  ...
  expect(data.node.updatedAt).toBe(graph.todos.alice1.updatedAt?.toISOString());
});
