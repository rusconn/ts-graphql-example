...
import type { TodoTitleQuery, TodoTitleQueryVariables } from "../_schema.ts";

const executeQuery = executeSingleResultOperation<TodoTitleQuery, TodoTitleQueryVariables>(
  /* GraphQL */ `
...
test("owned", async () => {
  ...
  expect(data.node.title).toBe(graph.todos.admin1.title);
});
