...
import type { TodoCreatedAtQuery, TodoCreatedAtQueryVariables } from "../_schema.ts";

const executeQuery = executeSingleResultOperation<TodoCreatedAtQuery, TodoCreatedAtQueryVariables>(
  /* GraphQL */ `
...
test("owned", async () => {
  ...
  expect(data.node.createdAt).toBe(graph.todos.admin1.createdAt?.toISOString());
});
...
test("not owned, but admin", async () => {
  ...
  expect(data.node.createdAt).toBe(graph.todos.alice1.createdAt?.toISOString());
});
